...
export default function MealsOverScreen({ route }) {
  const { width } = useWindowDimensions();
  const navigation = useNavigation();
  const name = { ...route.params };
  const meals = MEALS.filter((item) => item.categoryIds.includes(name.id));

  useLayoutEffect(() => {
    navigation.setOptions({
      title: CATEGORIES.find((item) => item.id === name.id).title,
    });
  }, [name.id, navigation]);
